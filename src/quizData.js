function parseChoiceCount(text) {
  ...
  const words = { one: 1, two: 2, three: 3, multiple: 99 };
  const explicit = lower.match(/\(choose\s+(\d+|one|two|three|multiple)\)/);
  if (explicit) {
    return Number(explicit[1]) || words[explicit[1]] || 1;
  }

  const implied = lower.match(/\bwhich\s+(one|two|three)\b/);
  if (implied) {
    return words[implied[1]] || 1;
  }
...
function cleanPrompt(text) {
  return text.replace(/\s+\(Choose [^)]+\)/i, "").trim();
}

function parseEntry(entry, order) {
  const lines = entry.split("\n").map((line) => line.trimEnd());
  const heading = lines.shift().trim();
  const questionNumber = (heading.match(/^Question\s+(\d+)/i) || [null, order + 1])[1];
  const contentLines = lines.join("\n").trim().split("\n").map((line) => line.trim()).filter(Boolean);
  const optionStartIndex = contentLines.findIndex((line) => /^[A-Z]\.\s+/.test(line));

  const prompt = optionStartIndex !== -1
    ? contentLines.slice(0, optionStartIndex).join(" ").trim()
    : (contentLines[0] || "");

  const options = optionStartIndex !== -1
    ? contentLines.slice(optionStartIndex).reduce((list, line) => {
        if (/^[A-Z]\.\s+/.test(line)) {
          list.push(line.replace(/^[A-Z]\.\s+/, "").trim());
        } else if (list.length) {
  ...
  return {
    ...
    choose: parseChoiceCount(prompt),
    options
  };
}

const QUIZ_PART_1 = String.raw`Question 1
...
Milestones track individual commits, while labels group entire repositories`;

const QUIZ_PART_2 = String.raw`Question 49
...
D. number of repositories owned`;

const QUIZ_PART_3 = String.raw`Question 97
...
const rawQuiz = [QUIZ_PART_1, QUIZ_PART_2, QUIZ_PART_3].join("\n\n");

export const quizQuestions = rawQuiz
